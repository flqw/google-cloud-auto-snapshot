...
gcloud compute disks list --format='value(name,zone)'| while read -r DISK_NAME ZONE; do
  gcloud compute snapshots create autogcs-"${DISK_NAME:0:31}"-"$(date '+%Y-%m-%d-%s')" --source-disk="${DISK_NAME}" --source-disk-zone="${ZONE}" --snapshot-type="${SNAPSHOT_TYPE:-ARCHIVE}" --storage-location="${STORAGE_LOCATION}"
done
#
